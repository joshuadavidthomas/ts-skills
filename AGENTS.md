...
- `internal/agentskill/` parses, validates, hashes, and identifies Agent Skill directories
- `internal/safetree/` validates and stages bounded, portable file trees
- `internal/server/` owns the daemon, tsnet node, catalog, SQLite storage, browser upload, HTTP API, and server-rendered UI
