...
Each release creates one archive containing both `ts-skills` and `ts-skillsd` for each target:
...
- `ts-skills_<os>_<arch>.tar.gz` (linux/darwin)
- `ts-skills_windows_<arch>.zip` (windows)
- `checksums.txt`

GitHub stores a signed provenance attestation for every file listed in `checksums.txt`. GoReleaser also updates the Homebrew tap repository with `Formula/ts-skills.rb`.
...
```bash
gh attestation verify ts-skills_linux_amd64.tar.gz --repo joshuadavidthomas/ts-skills
```
...
GoReleaser injects the release tag into `internal/version.Version` through `-ldflags` for both binaries. Local builds default to `dev`.

Both commands report the injected value:
