...
func Decode(ctx context.Context, archivePath, stagingParent string, limits safetree.Limits) (_ *safetree.Snapshot, err error) {
	if ctx == nil {
	...
	builder, err := safetree.NewBuilder(stagingParent, limits)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	for _, entry := range archive.File {
		...
		if entry.UncompressedSize64 > math.MaxInt64 {
			return nil, &safetree.LimitError{Limit: "file bytes", Max: limits.MaxFileBytes, Actual: math.MaxInt64}
		}
